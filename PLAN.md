...
- [ ] FSM pipeline: Intent Classifier → Router → Entity Extractor → **SlotState update** → Next-field policy → Response Generator
- [ ] Slot-filling cho cả G_1..G_5 (hỏi field thiếu theo priority, từ FSM — không phải LLM)
...
- [ ] Code **8 exception** trên nền FSM (cơ chế ở [TECHSTACK.md §9](TECHSTACK.md)):
...
- [ ] **Ablation study:** có/không FSM · có/không tune recall · Qwen vs Việt-tuned · laptop medium vs GPU large WER
...
|---|---|---|
| Sở hữu | DialogueEngine (FSM), 8 exception, **eval framework + report** | ASR/VAD/TTS + mic loop, **normalization số tiếng Việt**, domain corpus, Gradio + video |
...
**Track A (Hiệp) — "Bộ não":**
`dialogue/` (engine, state, categories, intent, extraction, exceptions, response, post_call) · `llm/` (ollama_client, prompts) · `eval/` (run_eval, metrics, report_template) · `tests/` (test_dialogue_state, test_field_extraction, test_exception_handling, test_final_output_schema) · `docs/EVALUATION_REPORT.md`
...
| Re-ask field đã confirm (mất điểm exception) | Cao | **Đã giải quyết bằng kiến trúc FSM** |
...
- **Đừng over-engineer.** FSM thuần > framework; static policy > RAG. Mỗi dòng code phải truy được về yêu cầu brief.
...
| LLM tự nhớ state → vỡ | FSM xác định, partial JSON sạch khi hangup |
...
*Bước tiếp theo: chốt kế hoạch này → dựng Blueprint chi tiết (từng node FSM, schema đầy đủ, contract 2 track) → cắt thành Task Graph (TASK-001…) chia theo Track A / Track B.*
